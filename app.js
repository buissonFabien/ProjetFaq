...
// 		},
// 		//When a request ends, and if there's no request still running, the loader is hidden
...
// 		},
// 		//When a request fails, and if there's no request still running, the loader is hidden
...
app.controller('getInfoCtrl', function($scope, $http, $modal, $log) {
  ...
  $http.get($scope.getUrl
    ...
    // $scope.tabWithoutStopWord = $scope.tab[i].answer.split(' ');
    // for(k=0;k<$scope.tabWithoutStopWord.length;k++)
    // 	for(var j=0; j< stopWords.length; j++){
    // 		if($scope.tabWithoutStopWord[k] == stopWords[j] || $scope.tabWithoutStopWord[k] == no_accent(stopWords[j])){
    // 			delete $scope.tabWithoutStopWord[k];
    // 		}
    // 	}
    // }
    // console.log($scope.tabWithoutStopWord.length)
...
//************************************//
  $scope.tfidf = function(text){
    ...
    $scope.poids = [];
    $scope.tabText = [] ;
    //on ignore les accents qu'a tapé l'utilisateur
    var regEx = /[áàâäãåçéèêëíìîñïóòôöõúùûüýÿæœÁÀÂÄÃÅÇÉÈÊËÍÌÎÏÑÓÒÔÖÕÚÙÛÜÝŸÆŒ]/
    $scope.tabText = no_accent(text);
    $scope.tabText = angular.lowercase($scope.tabText);
    $scope.tabText = $scope.tabText.replace(new RegExp("[^(a-zA-ZáàâäãåçéèêëíìîñïóòôöõúùûüýÿæœÁÀÂÄÃÅÇÉÈÊËÍÌÎÏÑÓÒÔÖÕÚÙÛÜÝŸÆŒ)]", "g"), ' ');
    $scope.tabText = $scope.tabText.split(' ');
	...
	// Si un mot entré par l'utilisateur est dans les StopWords on l'efface
	// $scope.tabText = $scope.tabText.replace(/[\.,-\/#!$%\^&\*;:{}=\-_`~()]/g,' ')
    for(var i=0; i< $scope.tabText.length;i++){
    	for(var j=0; j< stopWords.length; j++){
    		if($scope.tabText[i] == stopWords[j] || $scope.tabText[i] == no_accent(stopWords[j]) || $scope.tabText[i] == ""	) {
    			delete $scope.tabText[i];
    		}
    	}
    }
    ...
    for ( var i = 0; i < $scope.tabText.length; i++) {
      ...
      for (var j = 0; j < $scope.tab.length; j++ ) {
        ...
        var biFlags = 0;
  		...
  		$scope.tabAnswer = $scope.tab[j].answer;
        $scope.tabTitle = $scope.tab[j].title;
        $scope.tabAnswer = $scope.tabAnswer.concat($scope.tabTitle);
        $scope.tabAnswer = $scope.tabAnswer.replace(new RegExp("[^(a-zA-ZáàâäãåçéèêëíìîñïóòôöõúùûüýÿæœÁÀÂÄÃÅÇÉÈÊËÍÌÎÏÑÓÒÔÖÕÚÙÛÜÝŸÆŒ)]", "g"), ' ');
        $scope.tabAnswer = $scope.tabAnswer.split(' ');
        for(var k = 0; k < $scope.tabAnswer.length; k++) {
         	// $scope.tabAnswer[k] = $scope.tabAnswer[k].replace(new RegExp("[^(a-zA-ZáàâäãåçéèêëíìîñïóòôöõúùûüýÿæœÁÀÂÄÃÅÇÉÈÊËÍÌÎÏÑÓÒÔÖÕÚÙÛÜÝŸÆŒ)]", "g"), ' ');
         	$scope.tabAnswer[k] =  no_accent(angular.lowercase($scope.tabAnswer[k]));

			if ($scope.tabText[i] ==$scope.tabAnswer[k] 
				|| $scope.tabText[i] == $scope.tabAnswer[k]
				&& $scope.tabText[i] != "") {
				flags = 1;
			}
        }
    ...
    for (var i = 0; i < $scope.tabText.length; i++) {
      ...
      for (var j = 0; j < $scope.tab.length; j++ ){
        $scope.tf = 0;
        //on récupère les champs "answer" et "title" qu'on split par l'espace pour isoler chaque mot:
        ...
        //Pour chaque mots:
         for(var k = 0; k < $scope.tabAnswer.length; k++){
         	...
         	// console.log($scope.tabAnswer[k])
	          ...
	          if ($scope.tabText[i] == $scope.tabAnswer[k] && ($scope.tabText[i+1] == $scope.tabAnswer[k+1] 
	          	|| $scope.tabText[i+1] == $scope.tabAnswer[k+2] 
	          	|| $scope.tabText[i+1] == $scope.tabAnswer[k+3])){
	          	$scope.tf += 1;
	          	$scope.idArticle = $scope.tab[j].id;
	          }
        }
        ...
        for(var p = 0; p < tabDf.length; p++){
          if (tabDf[p].mot == $scope.tabText[i] && $scope.tf > 0 ) {
            var poid = {'id' : $scope.tab[j].id ,
                        ...
                        'weight': ($scope.tf*Math.log($scope.tab.length/tabDf[p].df+0.01)) };
                        // +Math.log((15*$scope.tab[j].nbViews)/100) // a voir car résultats un peu faussé
...
app.directive('ngStylecss', function(){
  return {
    
    template: '<link rel="stylesheet" href="http://netdna.bootstrapcdn.com/twitter-bootstrap/2.3.2/css/bootstrap-combined.min.css" />'
  }
})
